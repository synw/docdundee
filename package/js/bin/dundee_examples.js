...
(async () => {
  ...
  let baseDir = args.length > 0 ? args[0] : (process.cwd() + "/public/examples");
  ...
  if (!_data.has("index.json")) {
    try {
      ...
      throw new Error(`Can not write examples index file ${e}`)
    }
  }
  ...
  try {
    ...
    throw new Error(`Can not write examples file ${e}`)
  }
})();
